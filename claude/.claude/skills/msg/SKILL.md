...
description: 'Send or receive messages between Claude agents in separate repos (shared mailbox files — no network). Verbs: send <repo> <msg> | read [repo] | peek [repo] | list. Triggers on "check my inbox", "any messages", "tell <repo>", "message <repo>", "ask <repo>", "send <repo>". Verb details in the skill body.'
