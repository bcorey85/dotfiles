---
name: adr
description: Produce a durable decision record (ADR) for features that did NOT go through QRSPI — sources the "why" from an eng-spec doc, the conversation, and the branch diff. Use after shipping a small/medium feature that involved a real design decision. Triggers on "/adr", "write an ADR", "record this decision".
allowed-tools: [Bash, Read, Glob, Grep, Write, Edit, AskUserQuestion]
---

# ADR (non-QRSPI lanes)

`/q-finalize` produces the ADR for QRSPI tasks by collapsing the task folder. This skill covers everything else — `/eng-spec` features and small changes where a real decision was made but no artifact set exists.
...
If a QRSPI task FOLDER exists for this key, stop and point at `/q-finalize` instead — it owns that lane and deletes the folder when done.
...
3. Detect the PR the same way `/q-finalize` does: `gh pr view --json url,number,title` on the current branch; else `gh pr list --search "<KEY>" --state all`; else `(pending)`.
