...
- Comment the non-obvious **why**, never the what. Don't narrate code or restate a signature (that's the stale noise a docstring-on-everything rule produces). DO add a brief comment where intent isn't recoverable from names alone: a rule that must stay true or things break, a non-obvious contract, units, a gotcha, or why-this-not-the-obvious-approach (e.g. a `(created_at, id)` keyset tiebreak that prevents silently dropped rows). Follow the project's existing comment/JSDoc convention — don't impose JSDoc where the codebase doesn't use it.
- **Comment density cap (HARD RULE):** comments must never exceed ~10% of lines in any file you write or edit. Before adding a comment, ask: would a rename or a shorter function make it unnecessary? If the code needs a comment to be legible, the code is the problem — fix the code. A function that is more comment than code is a design smell, not documentation. When editing existing files, do not add comments to bring a file under the cap; if the file is already over, flag it in your report as a `REFACTOR CANDIDATE` — the smell is pre-existing and not your fix to make, but the next person should know.
- **Plain language only (HARD RULE).** Variable names, function names, comments, and log messages use widely-understood English. No obscure infrastructure jargon (`sidecar`, `hedwig`, `strangler`, `canary`), no acronym soup (`OOM`, `GC` without context), no framework-specific slang (`middleware` is fine, `interceptor-pipeline` is not). The test: would a competent developer unfamiliar with the specific infrastructure pattern understand the name on first read? If not, rename it. `proxy` beats `sidecar`. `cleanup` beats `janitor`. `retry` beats `hedwig`. If the pattern genuinely needs a domain term (you're writing a service mesh), define it once in a comment at the top of the file — then use the short name everywhere below.
