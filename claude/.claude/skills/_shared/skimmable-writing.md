...
Shared by `/eng-arch` and `/finalize` — the single source of truth for the skimmability rules. Engineers in problem-solving mode scan headings (NN/g layer-cake pattern); they don't read. Write to be skimmed: trigger layer-cake scanning, then earn commitment reading with trust signals.
