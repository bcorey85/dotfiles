...
| `starship`            | `~/.config/starship.toml`                                     | kanso mist prompt                                                                                                                                                                                                                                                           |
...
- **Theme**: **kanso** (webhooked/kanso.nvim) — kanso-mist (dark) / kanso-pearl (light). Dark bg `#22262d`, fg `#c5c9c7`, blue `#8ba4b0` primary accent, aqua `#8ea4a2`, green `#8a9a7b`, red `#c4746e`. Light base `#f2f1ef`, fg `#22262d`, blue `#4d699b` primary accent. tmux/nvim/Emacs share a light/dark toggle via `~/.cache/theme-mode` (see `scripts/.local/bin/theme-mode`); Emacs has no maintained nightfox port so it recolors bg/fg on modus-vivendi/modus-operandi instead. **Dark-only (no light toggle yet)**: starship, waybar, rofi, dunst, hyprland, hyprlock. GTK/Qt apps use Adwaita-dark.
