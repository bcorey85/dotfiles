...
2. **Aggregate with jq/awk in a single pass** (redirect to a temp file if long). Compute:
   ...
   - **Iteration distribution**: runs at `iter=1`, `2`, `oneshot`. A fat tail at 2 means the loop isn't converging first-pass — check whether the same findings recur.
...
| iter tail                                           | > 25% reach iter 2   | convergence problem; check `/fix` skips vs reviewer re-flags                                                                                                                                                                                 |
