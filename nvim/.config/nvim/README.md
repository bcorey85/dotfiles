...
- **Theme:** monokai-pro (loctvl842/monokai-pro.nvim, "pro" filter) — dark-only; pick with `theme-use <family> [mode]`; light/dark synced across nvim/ghostty by the `theme-mode` script; the family registry and its overrides live in `lua/config/theme-sync.lua`
