...
Every phase whose human gate was removed (Questions, Research, Structure) — plus the two that keep one (Design, Plan) — runs an automated quality review before its artifact flows downstream. This is the compensating control for the removed gates: an artifact is always checked by a human OR an agent, never neither. Reviews never wait on a human; they preserve flow while restoring the quality the gates used to provide.
...
3. **ISSUES** → send the issue list back to the producer for ONE revision, then re-review:
   - Subagent producers — Research (`qrspi-research`), Plan (`qrspi-plan`): re-dispatch with the same inputs **plus** the issue list.
   - Inline producers — Design, Structure: you revise the artifact yourself against the issues.
   - **Max 2 revision rounds.**
4. **Still ISSUES after 2 rounds** → ESCALATE and log as `ESCALATED`:
   - Research / Structure / Plan: STOP and surface the remaining issues to the user (Structure has no human gate, so an unresolvable structure problem is the one thing worth interrupting the auto-flow for).
   - Design: fold the remaining issues into the presentation you're about to show — the human is already looking.

## Isolation

Pre-design reviews (Research) are dispatched WITHOUT the ticket — pass ONLY the research file + questions file. Post-design reviews (Design, Structure, Plan) may include the ticket and any prior artifact. Never hand the research reviewer the ticket path.
...
REVIEW_METRICS_FILE="$HOME/.claude/qrspi-review.jsonl" bash ~/.claude/skills/review/log-review-metrics \
  key=<TICKET> phase=<questions|research|design|structure|plan> verdict=<PASS|ESCALATED> rounds=<n> issues=<m>
```
...
**Structure** — artifact `IQ-XXX-04-structure.md`; inputs: structure + design docs:
