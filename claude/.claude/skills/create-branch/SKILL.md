...
- **If a Jira ticket key was provided** (matches pattern `[A-Z]+-\d+`): fetch the ticket using `getJiraIssue` (Cloud ID from `docs/mcp-references/JIRA.md`) to get the summary. Build the branch name: `<TICKET-KEY>-<slugified-summary>` (lowercase, hyphens, max ~50 chars). Example: `TAS-20-dockerize-dev-environment`.
