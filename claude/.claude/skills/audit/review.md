...
- **Catches** — what `/review` logs on every run (via `log-review-metrics`): repo, iter, disposition counts (`fix`/`ask`/`nit`/`blocker`), outcome buckets (`fixed`/`skipped_fp`), `test_intent`, `culled`, `specialists`, `result`. Every row also carries `comment_noise`, always 0 and not reported. Rows dated before 2026-09-01 also carry `class_closed` and `fixed_classes`; both are retired and neither is reported — the stopping-rule receipt is prose in the review packet now, not telemetry.
...
2. **Aggregate with jq/awk in a single pass** (redirect to a temp file if long). Compute:
   ...
   - **Iteration distribution**: runs at `iter=0`, `1`, `2`. A run reaches 2 whenever its first review's `fix` findings got a coder, so 2 is no tail — read it beside the share of runs with `fixed > 0`. Count rows at `3` or `oneshot` apart: they predate the present counter.
...
| --------------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FP rate                                             | > 30%                | reviewer over-flagging; name the false-finding class from the transcripts before any suppression rule goes into code-reviewer                                                                                                                |
| nit volume                                          | avg > 4/run          | reporting bar too low                                                                                                                                                                                                                        |
| blocker share                                       | > 15% of `fix`       | flag used for emphasis; the diff reviewer has no blocker definition, so check the tagged lines against what the loop stops a phase for                                                                                                       |
| test_intent                                         | frequent > 0         | bug-pinning recurring; reinforce spec-first test writing in coders                                                                                                                                                                           |
| culled                                              | not trending to 0    | the test writer overproducing tests; name the test class from the transcripts before any budget rule goes into `test-writer`. Sustained 0 at healthy volume = the writer's volume holds                                                      |
...
| walkthrough / cc / pr-human `class=bug`             | any cluster          | reviewer missing real bugs; check whether a specialist-ownership bullet deferred them to a pass that did not run, before reaching for `+deep`                                                                                                |
