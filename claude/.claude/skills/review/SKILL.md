...
5. **Decide next steps** based on the review outcome. Severity gating has two tiers:
   ...
   - **If all clear on the auto-fix gate (no CRITICAL or HIGH issues remain)**: The convergence loop is done.
     ...
     - **Test-intent audit (conditional, one-shot)**: Opus-priced, so it fires only when BOTH gates pass — mere test-file churn gives it nothing to judge:
       1. **Assertion gate**: the diff **added or modified assertions** in a test file — check the actual hunks for a changed expected value or a new `expect`/assert. Ripple does not count: renamed imports/call sites, moved test files, helper/fixture/setup edits, snapshot/format churn.
       2. **Oracle gate**: run `bash ~/.claude/scripts/resolve-task-dir.sh` here first (one cheap call). Exit 0 → proceed. Exit 4 (no spec dir — weak oracle only) → skip unless `+deep` was passed; note "test-intent audit skipped: no spec oracle". Exit 3 → relay the directory question to the user.
