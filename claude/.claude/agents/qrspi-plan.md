...
6. Write the plan to `DIR/IQ-XXX-05-plan.md` following `~/.claude/skills/_shared/plan-format.md` **in full** — it is the single source of truth for the template, risk-tier semantics, and format rules (shared with `/eng-spec`; `/code` and `/q-verify` consume this format). Header links: include all four QRSPI artifact paths.
