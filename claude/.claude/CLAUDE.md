...
- Branch exit → `/preflight` (front door): mechanical `/stage` triage → test-intent audit (test files only) → `/verify` → `/orient` on branches with no spec → read-surface triage → receipt. No agent ever clears a semantic file for me to skip: only `/stage`'s deterministic SAFE tier is staged unread. I read the queue and stage, then `/commit`; `/adr` stays post-PR.
- Planning → `/eng-spec`. **One lane, no router.** It runs goal-blind research FIRST (`qrspi-questions` → `qrspi-leak-check` → `qrspi-research`, which never sees the ticket), then architect exploration, then decisions resolved with me one at a time. The research-before-design order is the whole point — never let a goal word reach the research agent, and never reorder it. Tag escapes with `/escape`; review via `/audit review`.
