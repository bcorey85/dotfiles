...
- **Theme:** three families — flexoki (kepano/flexoki-neovim, native; dark / light), token (ThorstenRhau/token, warm cream; dark / light), and flume (mitander/flume.nvim; dark = mira, light = mesa; the default); pick with `theme-use <family> [mode]`; light/dark synced across nvim/ghostty by the `theme-mode` script; the family registry and its overrides live in `lua/config/theme-sync.lua`
