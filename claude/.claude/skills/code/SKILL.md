...
2. **Detect multi-phase plans (MANDATORY check)**: If the task input is a path to a plan file (e.g., `*-plan.md` under `docs/plans/`) or pasted plan content, check whether it contains multiple `## Phase N:` sections.
   ...
   **If it's a multi-phase plan:**
   ...
   - **No per-phase test-intent audit** (LOOP-COST round 9: retired from the phase gate — the enforced coder/test-writer split severs bug-pinning's causal input, and round 8 measured 0 bug-pinning verdicts under a live audit costing ~24% of round spend). The cull/coverage half still runs at `/branch-recap`, and `/verify` still reconciles plan↔diff at branch end; a `weak`/`bug-pinning` finding surfacing from ANY other gate still routes to `test-writer` re-dispatch (implementation-blind), `/fix` only when it implicates src.
