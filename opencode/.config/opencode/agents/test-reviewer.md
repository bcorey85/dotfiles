...
- **"backend"** or **"be"**: Review all backend test files against their source modules. Detect test file locations by reading `AGENTS.md` and globbing for common patterns (`**/*.test.ts`, `**/*.spec.ts`, `**/tests.py`, `**/test_*.py`, etc.) in the backend directory.
...
- **Consider the test framework.** Read `AGENTS.md` and examine existing test files to determine which framework is used (Jest, Vitest, pytest, Django TestCase, Bun test, etc.). Align suggestions with that framework's idioms.
