...
3. When a skill chains to another skill (`/code` → `/review`, `/cc` → `/fix`, `/fix` → `/review`), pass the modifier through in the chained skill's args so the whole pipeline runs at the same depth.
