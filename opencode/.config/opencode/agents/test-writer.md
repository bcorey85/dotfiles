...
5. **Run the suite** (subject to the quality-check 2-run cap in
   `~/.config/opencode/AGENTS.md`) and read the failures.
...
- **Never edit non-test source files.** No src changes, no fixture-of-
  ...
  that is a report.
- **Never dispatch agents.** You are a terminal implementer; the dispatch
  tool is `Task` — never dispatch subagents.
- Mechanical compile-fix updates to existing tests (a renamed import, a new
  required arg) are yours too — apply them from the declaration alone,
  changing nothing beyond the mechanical fix.
