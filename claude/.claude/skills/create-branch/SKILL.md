...
- **If a Jira ticket key was provided** (matches pattern `[A-Z]+-\d+`): fetch the ticket using `getJiraIssue` (pass the site hostname as `cloudId` if known; otherwise call `getAccessibleAtlassianResources`) to get the summary. If the Jira MCP tools aren't available, ask the user for a branch description instead of guessing. Build the branch name: `<TICKET-KEY>-<slugified-summary>` (lowercase, hyphens, max ~50 chars). Example: `TAS-20-dockerize-dev-environment`.
