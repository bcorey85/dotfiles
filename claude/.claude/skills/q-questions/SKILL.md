...
(This mirrors the extraction in `~/.claude/scripts/qrspi-resolve-dir.sh` — that script resolves _existing_ task directories for later steps; this step creates the directory, so only the extraction is shared. Keep the regex in sync.)
