...
- **Theme:** kanagawa (rebelot/kanagawa.nvim, default) or onedark "darker" (navarasu/onedark.nvim) — pick with `theme-use <family> [mode]`; light/dark synced across nvim/tmux/ghostty by the `theme-mode` script; the family registry and its overrides live in `lua/config/theme-sync.lua`
