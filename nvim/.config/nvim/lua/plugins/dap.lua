...
return {
  ...
  -- Descriptions are duplicated from the map() calls in config() so which-key
  -- can label the menu WITHOUT loading dap. Keep the two in sync when editing.
  keys = {
    { "<leader>db", desc = "Toggle breakpoint" },
    { "<leader>dB", desc = "Conditional breakpoint" },
    { "<leader>dc", desc = "Continue / start" },
    { "<leader>dC", desc = "Run to cursor" },
    { "<leader>de", desc = "Set exception breakpoints" },
  ...
  config = function()
    ...
    local function configure()
      ...
      -- Per-project ".vscode/launch.json" configs are read automatically
      -- on-demand by nvim-dap (see :help dap-providers) — the old explicit
      -- load_launchjs() call is deprecated and no longer needed.
