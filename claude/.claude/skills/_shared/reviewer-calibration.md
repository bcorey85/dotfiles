...
- **Do not trust stale tool state.** LSP diagnostics and TS-server snapshots can reference deleted files, unfinished mid-edit state, or imports that actually resolve. Before flagging a type/import error, reconcile against the filesystem and a fresh typecheck of the files in question — a green typecheck beats a red cached diagnostic. Never run the full suite or a long gate; the review loop runs it once, at the end.
