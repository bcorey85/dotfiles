...
3. **Interpret** — flag, with thresholds:
   ...
   - **Escape flags** (the trust dial — these decide where human attention stays mandatory):
     ...
     - Escapes carrying `guard=none` (or missing `guard` entirely) → the ratchet in `/escape` step 3 never fired. An escape without a guard is bookkeeping, not a closed loop; these are the rows to work through first.
