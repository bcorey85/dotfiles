...
**Status**: built 2026-06-12 — `SKILL.md` (62-line dispatcher), four `qrspi-*` agents (`~/.claude/agents/`), inline phase playbooks (`design-phase.md`, `structure-phase.md`). **Verdict (2026-07-06): keep, with scope note — first eval run (n=1)** — retrospective A/B vs `/eng-spec` on a shipped regret ticket (datachat tool-persistence bug; full record: `~/dev/plan-ab/.sealed/datachat-tool-history/ANSWER-KEY.md`). Both lanes avoided the documented regret (P1 pass both), so per the pre-registered rule the ceremony delta was NOT justified by design quality on this ticket; /eng-spec additionally named the API constraint explicitly and matched the product's eventual direction. QRSPI's measured wins were process properties — main-thread context economy (~57% after a full pipeline), risk-tier gating that downstream `/code` auto-advance respects, audit trail — at ~2.3× output tokens and ~2.2× wall clock. Standing guidance: prefer `/eng-spec` for tickets whose risks are general-API knowledge; reserve `/q-plan` for multi-phase features where research contamination or phase gating carries real weight (its original charter). Rematch condition before any stronger claim: a regret ticket with a codebase-specific trap, and/or `qrspi-research` pinned to opus. Calibration follow-ups from the run: qrspi-research cites loosely (3/3 research phases drew citation issues); qrspi-review burned a round on cosmetic line-drift (add a "cosmetic ≠ blocking" rule); design phase never has to name external API contracts it relies on (P2 miss) — consider adding that prompt line.
