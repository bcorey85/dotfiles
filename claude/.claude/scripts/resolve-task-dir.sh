#!/usr/bin/env bash
# resolve-task-dir.sh — resolve the /eng-spec task directory under docs/plans/
# Shared by /verify, /branch-recap, /code, and /adr. Do NOT reimplement inline.
#
# docs/eng-specs/ is the pre-rename root; both are searched so repos that have
# not migrated keep resolving. Drop the legacy root once none remain.
#
...
# exit 5: no task dir, exactly one legacy flat plan FILE, printed
...
count=0
found=""
for d in "$specs/$ticket/" "$specs/$ticket"-*/ "$legacy/$ticket/" "$legacy/$ticket"-*/; do
  [ -d "$d" ] || continue
  count=$((count + 1))
  found="${found}${d%/}
"
...
fcount=0
ffound=""
for f in "$specs/$ticket"*.md "$legacy/$ticket"*.md; do
  [ -f "$f" ] || continue
  fcount=$((fcount + 1))
  ffound="${ffound}${f}
"
...
case "$fcount" in
  0) echo "no task directory or plan matching $specs/$ticket* (or $legacy/$ticket*)" >&2; exit 4 ;;
