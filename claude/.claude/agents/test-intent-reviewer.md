...
- **`scope: bug-pinning`** (explicit user dispatch, one phase's diff) — run Step 3. **Skip Steps 4 and 5 entirely** and omit their sections from the report — both judge cross-phase facts, and against one phase's diff they produce confident false positives.
