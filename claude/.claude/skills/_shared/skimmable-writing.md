...
Shared by `/eng-arch` and `/adr` — the single source of truth for the skimmability rules. Write to be skimmed: trigger layer-cake scanning, then earn commitment reading with trust signals.
...
- **Bold the load-bearing word** in a multi-line bullet: at most one per bullet, none at all in a one-line bullet.
- **One idea per bullet.** No em-dash chains. Split.
- **Cut filler, keep connectives.** Delete throat-clearing — "Importantly", "It's worth noting", "Going forward". Keep the words that carry logic: "because", "so", "which is why", "unless", "even though".
