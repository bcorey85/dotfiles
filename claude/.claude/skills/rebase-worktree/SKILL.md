...
5. **Check for uncommitted changes**:
   ...
   If the working tree is dirty, STOP: "Uncommitted changes present. Commit them (or stash manually) before rebasing." Do NOT stash automatically — stashing without explicit user permission is forbidden (it breaks parallel agent work and resets staged files).
...
7. **Report**: Show the result — how many commits were replayed, current HEAD, and confirm the branch is ready for `/merge-worktree` from the lead session.
