...
- **Theme**: **onedark "darker"** (navarasu/onedark.nvim, `#1f2329`) is the only family — one theme, locked in. The `theme-mode` script (scripts package) still owns both axes: **family** (`~/.cache/theme-family`: `onedark-darker`; `theme-mode list`; `theme-mode use <family> [mode]`) × **mode** (`~/.cache/theme-mode` dark/light, toggled by `theme-mode` / tmux `prefix T` / nvim `<leader>ut` / Emacs `SPC t t`). Consumers: tmux sources `~/.config/tmux/<family>-<mode>.conf`; nvim polls both state files (lua/config/theme-sync.lua — its FAMILIES table maps schemes and owns all theme-reactive overrides: markdown headings, prose calm, gitsigns word-diff, the per-family comment-contrast fixup); ghostty gets `theme-switch.conf` (machine-local include written by the script; reload ctrl+shift+, — custom palettes live in `ghostty/.config/ghostty/themes/`); Emacs is mode-only (doom-one / doom-one-light from doom-themes — same family, stock `#282c34` rather than the terminal side's `#1f2329`); starship is mode-only too (theme-mode regenerates the REAL `~/.config/starship.toml` — starship's default path — from the stowed `~/.config/starship.toml.template` with the active `dark`/`light` palette; starship has no include but re-reads that path each prompt, so every open shell flips live with no reload and no STARSHIP_CONFIG. `.zshrc` bootstraps the file from the template on a fresh machine). Adding a family = 4 touchpoints, listed in the `theme-mode` header. **Dark-only, no mode switching**: waybar, rofi, dunst, hyprland, hyprlock — all onedark-darker hex values (`#1f2329` canvas, `#181b20` inset, `#a0a8b7` fg, `#858c9a` muted, `#4fa6ed` accent), hardcoded rather than read from the state files. GTK/Qt apps use Adwaita-dark.
