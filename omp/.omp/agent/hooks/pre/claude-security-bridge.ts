...
const PRE_TOOL_GATES: Record<string, { tool: string; gates: string[] }> = {
	bash: { tool: "Bash", gates: ["bash-safety-gate.sh", "git-discipline-gate.sh", "review-commit-gate.sh"] },
...
interface HookPi {
	on(event: "tool_call", handler: (event: ToolEvent, ctx: HookCtx) => Promise<{ block: boolean; reason?: string } | undefined>): void;
	on(event: "tool_result", handler: (event: ToolEvent, ctx: HookCtx) => Promise<undefined>): void;
}
 ...
 */
function runGate(script: string, payload: Record<string, unknown>, cwd: string): Promise<GateResult> {
	...
	try {
		child = spawn("bash", [join(SCRIPTS_DIR, script)], { cwd, stdio: ["pipe", "pipe", "pipe"] });
	} catch (e) {
	...
	timer = setTimeout(() => {
		...
		console.error(`[claude-security-bridge] ${script} timed out after ${GATE_TIMEOUT_MS}ms — failing open`);
		finish({ denied: false });
	}, GATE_TIMEOUT_MS);
	...
	child.on("close", (code) => {
		...
		for (const line of stdout.split("\n")) {
			...
			try {
				...
				if (isHookVerdict(parsed) && parsed.hookSpecificOutput?.permissionDecision === "deny") {
					return finish({
						denied: true,
						reason: parsed.hookSpecificOutput.permissionDecisionReason ?? `${script} denied`,
					});
				}
			} catch {
		...
		if (code === 2) {
			return finish({ denied: true, reason: stderr.trim() || `${script} blocked` });
		}
		finish({ denied: false });
	});
...
export default function bridge(pi: HookPi): void {
	pi.on("tool_call", async (event, ctx) => {
		...
		for (const toolInput of inputs) {
			...
			const results = await Promise.all(mapping.gates.map((g) => runGate(g, payload, cwd)));
