...
1. Read `AGENTS.md` at the project root for the stack, conventions, and commands.
...
- **Review handoff (last line of your report)**: end with `REVIEW: recommended — <changed files>` for any non-trivial change, or `REVIEW: skip (trivial)` for a typo / single-line / rename / comment-only edit. This is the orchestrator's cue to run `/review` before `/commit` — a direct `Task` dispatch does not auto-review, so make the cue impossible to miss.
