...
- **Theme:** onedarkpro (olimorris/onedarkpro.nvim; onedark / onelight) by default, picked with `theme-mode use <family>`; light/dark synced across nvim/ghostty by the `theme-mode` script; the family registry and its overrides live in `lua/config/theme-sync.lua`
