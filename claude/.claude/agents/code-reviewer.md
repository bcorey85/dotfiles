...
- **Low-value tests introduced by this diff (`[test-fluff]`).** A test ADDED in the change under review that cannot fail for a reason a user cares about — it inflates the diff without buying regression protection. Flag on structure, MEDIUM severity, prefix the finding with `[test-fluff]`, and name the fix (prune it, or tighten it to assert real behavior). The patterns:
