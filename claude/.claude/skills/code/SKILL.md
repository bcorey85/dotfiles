...
2. **Detect multi-phase plans (MANDATORY check)**: If the task input is a path to a plan file (e.g., `*-plan.md` under `docs/eng-specs/`) or pasted plan content, read it and check whether it contains multiple `## Phase N:` sections.
   ...
   **If it's a multi-phase plan:**
   ...
   - After the coder completes and you summarize, auto-dispatch `/peer-review` (step 5).
...
6. **After coder(s) complete**, summarize for the user AND build a handoff block for downstream review.
...
7. **Auto-dispatch peer review**: After summarizing the coder output, tell the user: "Auto-dispatching `/peer-review` to check the implementation before committing." Then invoke the `/peer-review` skill via the Skill tool with `skill: "peer-review"` and `args` containing the handoff block from step 5 plus any `+fast`/`+deep` modifier. This runs AFTER all coders have completed and the summary is presented. For parallel fullstack dispatches, both coders finish before this step runs — that is the correct sequencing.

8. **Multi-phase plans only — pause for sign-off**: If step 2 detected a multi-phase plan, after `/peer-review` returns, stop and print the phase-complete block (see below) with all placeholders resolved. Do not auto-advance. When the user replies to continue (in a fresh context after `/clear`, or in the same context if they skipped clearing), re-enter at step 2 with the next phase — use `git status` and the plan's success criteria to detect what's already done.

## Phase-Complete Block

After each phase + peer-review completes, print this block verbatim with `<N>`, `<N+1>`, `<plan-path>`, and the manual-verification list filled in from the just-finished phase. The user copy-pastes the slash command after `/clear`.
