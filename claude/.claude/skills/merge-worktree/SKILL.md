...
5. **Check for reverts** — look for deletions of files that exist on the current branch:
   ...
   ```bash
   git diff <current-branch>..worktree-<name> --name-only
   ```

   (Mentally exclude generated/codemod/docs-only paths if the project has them — don't hardcode exclusions into the command.)
